...
Value Eval::evaluate(const Position& pos) {
    ...
    constexpr int lazyThresholdSimpleEval = 2550;
    constexpr int lazyThresholdSmallNet   = 1050;
    bool          lazy                    = std::abs(simpleEval) > lazyThresholdSimpleEval;

    if (lazy)
        v = simpleEval;
    else
    {
        bool smallNet = std::abs(simpleEval) > lazyThresholdSmallNet;
